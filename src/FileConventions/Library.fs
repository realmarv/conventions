...
let private WrapParagraph (text: string) (maxCharsPerLine: int) : string =
    ...
    let rec processWords (currentLine: string) (wrappedText: string) (remainingWords: List<Text>) : string =
        ...
        | word :: rest ->
            ...
            | "", _ ->
                processWords word.Text wrappedText rest
            | _, { Type = PlainText } when String.length currentLine + word.Text.Length + 1 <= maxCharsPerLine ->
                processWords (currentLine + " " + word.Text) wrappedText rest
            | _, { Type = PlainText } ->
                processWords word.Text (wrappedText + currentLine + Environment.NewLine) rest
            | _, _ ->
                processWords "" (wrappedText + currentLine + Environment.NewLine + word.Text + Environment.NewLine) rest
