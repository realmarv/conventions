...
module.exports = {
    ...
    rules: {
        ...
        'body-soft-max-line-length': [RuleStatus.Error, 'always', bodyMaxLineLength],
...

        {
            rules: {
                ...
                'header-max-length-with-suggestions': ({header}: {header:any}, _: any, maxLineLength:number) => {
                    let headerStr = Helpers.convertAnyToString(header, "header");
                    return Plugins.headerMaxLengthWithSuggestions(headerStr, maxLineLength);
                },
                ...
                'body-soft-max-line-length': ({raw}: {raw:any}, _: any, maxLineLength:number) => {
                    let rawStr = Helpers.convertAnyToString(raw, "raw").trim();
