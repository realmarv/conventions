...
|> Seq.iter(fun commit ->
    ...
    let checkSuitsJsonString = (client.GetStringAsync url).Result

    if checkSuitsJsonString.Contains "\"check_suites\":[]" then
        let errMsg =
            sprintf
    ...
    let checkSuitsParsedJson = checkSuitsType.Parse checkSuitsJsonString

    let commitMessageTitle =
        (checkSuitsParsedJson.CheckSuites.[0]
            .HeadCommit
            .Message)
            .Split(
            "\n"
    ...
    let commitHash = checkSuitsParsedJson.CheckSuites.[0].HeadSha
    let status = checkSuitsParsedJson.CheckSuites.[0].Status
    let conclusion = checkSuitsParsedJson.CheckSuites.[0].Conclusion

    if status = "completed" then
        if commitMessageTitle.Contains "failing test" && conclusion = "success" then
            Console.Error.WriteLine
                $"The CI status of the commit \"{commitHash}\" must be `failure`, since it's adding a failing test."

            Environment.Exit 2

    else if conclusion = "failure" then
        Console.Error.WriteLine
            $"The CI status of a commit that doesn't add a failing test must be `success`."
